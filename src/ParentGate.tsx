// src/ParentGate.tsx (Native)
import { useMemo, useState } from 'react';
import { View, Text, TextInput } from 'react-native';
import { checkParentPin } from './parentPin';
...
export function ParentGate({ onExit, onUnlocked }: Props) {
  const { t, dir } = useI18n();
  ...
  return (
    <View style={{ padding: 16 }}>
      ...
      <View style={{ marginTop: 16, gap: 10 }}>
        <Text style={{ fontSize: 13, opacity: 0.75 }}>{t('parent.gate.subtitle')}</Text>
        ...
        <Button onClick={() => setShowPin((v) => !v)} disabled={locked}>
          {showPin ? t('parent.gate.button.hide') : t('parent.gate.button.show')}
        </Button>

        {error ? <Text style={{ color: '#b00020', fontSize: 13 }}>{error}</Text> : null}

        <Button variant="primary" fullWidth onClick={submit} disabled={locked}>
